...
const EMPTY = { bank_name: '', address: '', city: '', phone: '', email: '', is_active: 'Y' };
...
export default function Banks() {
  ...
  const fetchItems = useCallback(() => {
    setLoading(true);
    axios.get(API).then(r => { setItems(r.data); setError(null); }).catch(() => setError('Failed to load blood banks.')).finally(() => setLoading(false));
  }, []);
  ...
  function openEdit(b) {
    setEditingId(b.BANK_ID);
    setForm({ bank_name: b.BANK_NAME, address: b.ADDRESS || '', city: b.CITY || '', phone: b.PHONE || '', email: b.EMAIL || '', is_active: b.IS_ACTIVE || 'Y' });
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editingId) { await axios.put(`${API}/${editingId}`, form); setSuccess('Blood bank updated.'); }
      else           { await axios.post(API, form);                  setSuccess('Blood bank added.');   }
  ...
  async function handleDelete(b) {
    if (!window.confirm(`Delete "${b.BANK_NAME}"? This cannot be undone.`)) return;
    try {
      await axios.delete(`${API}/${b.BANK_ID}`);
      setSuccess('Blood bank deleted.'); setTimeout(() => setSuccess(null), 4000); fetchItems();
    } catch (err) { setError(err.response?.data?.error || 'Failed to delete. It may have associated records.'); }
  }
    ...
    <div style={{ padding:'28px 32px', fontFamily:'inherit' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'24px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
          <Landmark size={26} color="#8B0000" />
          <div>
            <h1 style={{ margin:0, fontSize:'1.45rem', fontWeight:700, color:'#111827' }}>Blood Banks</h1>
            <p style={{ margin:0, fontSize:'0.82rem', color:'#6b7280' }}>{items.length} blood banks</p>
          </div>
        </div>
        <button style={s.btn} onClick={openCreate}><Plus size={16} />Add Bank</button>
      </div>
      ...
      <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #e5e7eb', overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        {loading ? (
          ...
          <div style={{ padding:'48px', textAlign:'center', color:'#6b7280' }}>No blood banks yet.</div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.875rem' }}>
            <thead>
              <tr style={{ background:'#f9fafb', borderBottom:'1px solid #e5e7eb' }}>
                {['Bank Name','City','Phone','Email','Active','Actions'].map(h => <th key={h} style={s.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {items.map(b => (
                <tr key={b.BANK_ID} style={{ borderBottom:'1px solid #f3f4f6' }}>
                  <td style={{ ...s.td, fontWeight:600, color:'#111827' }}>{b.BANK_NAME}</td>
                  <td style={{ ...s.td, color:'#374151' }}>{b.CITY || '—'}</td>
                  <td style={{ ...s.td, color:'#374151' }}>{b.PHONE || '—'}</td>
                  <td style={{ ...s.td, color:'#374151' }}>{b.EMAIL || '—'}</td>
                  <td style={s.td}>
                    <span style={{ background: b.IS_ACTIVE === 'Y' ? '#dcfce7' : '#fee2e2', color: b.IS_ACTIVE === 'Y' ? '#166534' : '#991b1b', padding:'3px 10px', borderRadius:'99px', fontSize:'0.74rem', fontWeight:600 }}>
                      {b.IS_ACTIVE === 'Y' ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td style={s.td}>
                    <div style={{ display:'flex', gap:'6px' }}>
                      <button style={s.edit} onClick={() => openEdit(b)}><Pencil size={12} />Edit</button>
                      <button style={s.del}  onClick={() => handleDelete(b)}><Trash2 size={12} />Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showForm && (
        <div style={s.ov} onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div style={s.pan}>
            <div style={s.ph}>
              <h2 style={{ margin:0, fontSize:'1.1rem', fontWeight:700 }}>{editingId ? 'Edit Blood Bank' : 'Add Blood Bank'}</h2>
            ...
            <form onSubmit={handleSubmit}>
              <div style={s.grid}>
                <label style={{ ...s.label, gridColumn:'1 / -1' }}>Bank Name *<input name="bank_name" value={form.bank_name} onChange={ch} required style={s.inp} placeholder="National Blood Bank" /></label>
                <label style={s.label}>City<input name="city" value={form.city} onChange={ch} style={s.inp} /></label>
                ...
                <label style={s.label}>Status
                  <select name="is_active" value={form.is_active} onChange={ch} style={s.inp}>
              ...
              <div style={{ display:'flex', gap:'10px', padding:'4px 20px 0', justifyContent:'flex-end' }}>
                ...
                <button type="submit" disabled={submitting} style={{ ...s.btn, opacity: submitting ? 0.7 : 1 }}>
                  {submitting ? 'Saving…' : (editingId ? 'Update' : 'Add Bank')}
                </button>
              </div>
            </form>
          </div>
